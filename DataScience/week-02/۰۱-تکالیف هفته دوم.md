...
# بخش اول:‌ گزارش استقرار آزمایشگاه 
## جزئیات عملیاتی
...
- سرویس‌های موجود در راه‌حل:
    ...
    - پایگاه داده SQL: PostgreSQL 16
    - زمان‌بندی و مدیریت: Airflow 3.1.1
    - زمان‌بندی و مدیریت: NiFi 1.27.0
- نسخه گزارش: 0.2
- تاریخ آخرین نگارش: ۷ آبان‌ماه ۱۴۰۴
## درباره مشکل
...
# درباره الگوهای Cron
یک عبارت کرون از پنج بخش ساخته شده است، که هر کدام از این پنج مورد در جدول زیر توضیح داده شده است.

| *             | *             | *           | *           | *               |
| ------------- | ------------- | ----------- | ----------- | --------------- |
| دقیقه ۰ تا ۵۹ | ساعت ۰ تا  ۲۳ | روز ۱ تا ۳۱ | ماه ۱ تا ۱۲ | روز هفته ۰ تا ۶ |
در جدول زیر چند مثال از قواعد بالا آمده است.

| عبارت cron  | برنامه زمانی          |
| ----------- | --------------------- |
| * * * * *   | هر دقیقه              |
| 0 * * * *   | هر ساعت               |
| 0 0 * * *   | هر روز ۱۲ ظهر         |
| 0 0 * * FRI | جمعه‌ها ۱۲ ظهر        |
| 0 0 1 * *   | ۱۲ ظهر روز اول هر ماه |

# درباره Data modelهای data warehouse
در **انبار داده (Data Warehouse)**، **مدل داده (Data Model)** ساختاری است که نحوه‌ی سازمان‌دهی، ذخیره‌سازی و ارتباط داده‌ها را مشخص می‌کند تا تحلیل و گزارش‌گیری آسان‌تر شود. سه نوع اصلی مدل داده وجود دارد:

1. **مدل ستاره‌ای (Star Schema)** که شامل یک جدول واقعیت (Fact Table) در مرکز و چندین جدول بُعد (Dimension Tables) پیرامون آن است و برای سادگی و سرعت در تحلیل کاربرد دارد.
2. **مدل دانه‌برفی (Snowflake Schema)** که نسخه نرمال‌شده‌تری از مدل ستاره‌ای است و در آن جداول بُعد به جداول فرعی تفکیک می‌شوند تا تکرار داده‌ها کاهش یابد.
3. **مدل گلکسی یا چندبُعدی (Galaxy Schema)** که شامل چندین جدول واقعیت است و برای پشتیبانی از چند حوزه‌ی کسب‌وکار استفاده می‌شود.
    

هدف نهایی این مدل‌ها، بهینه‌سازی داده‌ها برای **تحلیل، گزارش‌گیری و تصمیم‌گیری مدیریتی** است.